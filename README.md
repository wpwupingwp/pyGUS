...
The `mode 4` requires one or more target images (`-images`). Users select two
reference and target regions by mouse and press any key to continue to next
...
It depends on hardware. Normally one image cost seconds. The program does not
require much memory or disk space.
...
* Q: It says my input is invalid, but I'm sure it's OK!
  ...
  A: Please check your files' path. The `space` or Chinese characters in the
  folder name or filename may cause this error.
